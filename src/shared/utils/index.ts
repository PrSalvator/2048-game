 ...
 */
const getPixelPositionFromCoordinates = ({ row, column }: ICoordinates, tileSize: number, gap: number) => {
...
const initializeGrid = (): IGridTilesIds => {
    return Array.from({ length: TILES_PER_ROW_COUNT }, () => Array(TILES_PER_ROW_COUNT).fill(undefined));
};
...
const checkMovePossibility = (tilesMap: ITileMap): boolean => {
    ...
    if (tilesArray.length < TILES_PER_ROW_COUNT * TILES_PER_ROW_COUNT) {
        return true;
    }
...
const getEmptyTilesCoordinates = (grid: IGridTilesIds): ICoordinates[] => {
    ...
    for (let row = 0; row < TILES_PER_ROW_COUNT; row++) {
        for (let column = 0; column < TILES_PER_ROW_COUNT; column++) {
            ...
            if (isNil(tile)) {
                emptyTilesCoordinates.push({ row, column });
            }
        }
    }
...
const getBreakpointFromWindowWidth = () => {
    ...
    if (width >= EBreakPoint.DESKTOP) {
        return EBreakPoint.DESKTOP;
    }
    if (width >= EBreakPoint.LAPTOP) {
        return EBreakPoint.LAPTOP;
    }
